...
export interface Discord {
  ...
  channelId?: string
  /**
   * Discord transport that produced the canonical input event.
   *
   * Voice inputs have already passed the active voice-session consent gate.
   */
...
export type DiscordMemoryCommandAction
  = | 'privacy'
...
export interface DiscordMemoryCommandEvent {
  commandId: string
...
export interface DiscordMemoryCommandResultEvent {
  commandId: string
  action: DiscordMemoryCommandAction
  sessionId: string
  status: 'ok' | 'error'
...
export interface InputMessageOverrides {
  ...
  /**
   * Explicitly prevents this turn from being persisted by cloud-sync consumers.
   *
   * Discord voice turns use this boundary because recorded speech must not leave
   * the disclosed provider path through a second, unrelated synchronization path.
   */
  cloudSync?: 'disabled'
}

/** Stable correlation and deadline shared by every boundary of one chat turn. */
export interface ChatTurnCorrelation {
  /** Globally unique identifier generated by the ingress adapter. */
  id: string
  /** Monotonic generation within the exact session. */
  generation: number
...
export interface WebSocketEventInputTextBase {
  text: string
  /** Required for Discord inputs; local Stage callers may omit it. */
...
export interface WebSocketEventInputVoiceBase {
  audio: ArrayBuffer
  /** Required for external async inputs; local Stage callers may omit it. */
  turn?: ChatTurnCorrelation
  overrides?: InputMessageOverrides
  contextUpdates?: InputContextUpdate[]
}
...
/** Invalidates one exact queued or running chat turn without affecting newer turns. */
export interface ChatTurnCancelEvent {
  /** Correlation copied from the original input event. */
  turn: ChatTurnCorrelation
  /** Exact chat session that owns the turn. */
  sessionId: string
  /** Lifecycle boundary that invalidated the turn. */
  reason: 'deadline' | 'disconnect' | 'disabled' | 'reset'
  /** Timestamp at which ingress requested cancellation. */
  cancelledAt: number
}
...
/**
 * Authenticates and binds one websocket peer to an immutable module identity.
 *
 * The server validates the token and requested module together. Subsequent
 * announcements and business events cannot replace this identity.
 */
export interface ModuleAuthenticateEvent {
  token: string
  module: {
    /** Module registry name requested by this peer. */
    name: string
    /** Optional registry index for explicitly indexed module instances. */
    index?: number
    /** Exact runtime identity requested by this peer. */
...
export const chatTurnCancel = defineProtocolEventa<ChatTurnCancelEvent>('chat:turn:cancel', {
  metadata: {
    delivery: {
      mode: 'consumer-group',
      group: 'chat-ingestion',
...
export const discordMemoryCommand = defineProtocolEventa<DiscordMemoryCommandEvent>('discord:memory:command', {
  metadata: {
    delivery: {
      mode: 'consumer-group',
...
export const discordMemoryCommandResult = defineProtocolEventa<DiscordMemoryCommandResultEvent>('discord:memory:command:result')
...
export const protocolEventMetadataByType = {
  ...
  [chatTurnCancel.id]: chatTurnCancel.metadata,
  [discordMemoryCommand.id]: discordMemoryCommand.metadata,
...
export interface ProtocolEvents<C = undefined> {
  ...
  'chat:turn:cancel': ChatTurnCancelEvent

  'discord:memory:command': DiscordMemoryCommandEvent
  'discord:memory:command:result': DiscordMemoryCommandResultEvent
