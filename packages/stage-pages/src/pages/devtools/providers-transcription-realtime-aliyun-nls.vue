...
async function startRecording() {
  ...
  const transcriptionResult = streamAliyunTranscription({
    ...createAliyunNLSProvider(
      ...
      onSessionTerminated: (error) => {
        if (error) {
          appendLog(`Session terminated: ${errorMessageFrom(error) ?? String(error)}`, 'error')
  ...
  transcriptionResult.text
    ...
    .catch((error) => {
      ...
      if (error instanceof DOMException && error.name === 'AbortError')
        ...
        appendLog(`Transcription failed: ${errorMessageFrom(error) ?? String(error)}`, 'error')
    })
    .finally(() => {
      isTranscribing.value = false
  ...
  try {
    ...
    console.error(error)
    appendLog(`Failed to start recording: ${errorMessageFrom(error) ?? String(error)}`, 'error')
