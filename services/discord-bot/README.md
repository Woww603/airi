...
AIRI_DISCORD_MEMORY_MAX_PREFERENCE_RECORDS='4096'
AIRI_DISCORD_MEMORY_MAX_FILE_BYTES='1048576'
...
`AIRI_DISCORD_MEMORY_CONSENT_REQUIRED='true'` is the default for guild sessions. A DM starts with memory enabled only while its preference store loads successfully, remains within the configured limits, and the turn supplies a non-empty stable Discord user id. Missing, blank, or runtime-invalid DM identity fails closed: it cannot recall or capture durable memory, mutate a preference, or fall back to session/channel identity; memory commands return a bounded identity error. DM persistence is keyed by the stable Discord user id and stores only explicit opt-outs: `off` creates one durable user-level opt-out, while `on` removes it. Opt-outs do not expire and are never evicted. Guild consent remains exact-session scoped: guild sessions do not read, inject, or extract memory before consent. Commands are `!airi memory on`, `!airi memory off`, `!airi memory status`, and `!airi memory forget`; Chinese equivalents are `记忆 开启`, `记忆 关闭`, `记忆 状态`, and `记忆 遗忘`. `forget` removes captured memories, including superseded provenance, for the exact session and persists the applicable opt-out. The preference store accepts at most `AIRI_DISCORD_MEMORY_MAX_PREFERENCE_RECORDS` records (4096 by default) and `AIRI_DISCORD_MEMORY_MAX_FILE_BYTES` serialized bytes (1 MiB by default). Invalid limits fail startup; malformed schema-v4 records, malformed/unreadable/oversized stores, and ambiguous legacy migrations preserve the original file and make affected memory decisions fail closed. Schema-v4 records are never silently discarded or rebuilt. Explicit `记住...` / `remember...` requests still have a safe source-text fallback if structured extraction is temporarily unavailable.
