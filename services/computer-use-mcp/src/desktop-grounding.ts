 ...
 * @param params.config - Computer-use configuration for accessibility capture.
 * @param params.executor - Desktop executor used for screenshots and window enumeration.
 * @param params.input - Optional observation controls.
 * @param params.extensionBridge - Optional Chrome extension semantics bridge.
 * @param params.cdpBridge - Optional Chrome DevTools Protocol semantics bridge.
...
export async function captureDesktopGrounding(params: {
  ...
  // Phase 2: Chrome semantic data (when available and allowed)
  ...
  if (shouldCaptureChrome) {
    ...
    if (chromeSemanticSnapshot?.pageTitle) {
